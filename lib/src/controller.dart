...
/// A Clean Architecture [Controller]. Should be aggregated
/// within a `ViewState` or a `View`. However, it is preferable to be
/// contained inside the `View` for readability
/// and maintainability.
///
/// The [Controller] handles the events triggered by the `View`.
/// For example, it handles the click events of buttons, lifecycle,
/// data-sourcing, etc...
///
...
///           CounterState(CounterController());
///     }
///
///     // ***************** ViewState *****************
...
abstract class Controller extends ChangeNotifier
  ...
  /// _refreshes the ControlledWidgets and the StatefulWidgets that depends
  /// on [CleanArchitectureApp.controller] of the [View] associated with the
  /// [Controller] if it is still mounted.
  @protected
  void shake() {
    if (_isMounted) {
      notifyListeners();
  ...
  /// Unmounts the [Controller] from the `View`.
  /// Called by the `View` automatically.
  /// Any cleaning, disposing should go in here.
  ///
  /// To perform correct actions that depends on latest [BuildContext] used on
  /// view before dispose, you must
  /// use injected context.
  ///
  /// The usage of [Controller.getContext] will be impossible here. Since the
  /// context will be null after the widget be removed from the widget
  /// tree.
  @mustCallSuper
  void onDisposed() {
  ...
  @protected
  GlobalKey<State<StatefulWidget>> getStateKey() => _globalKey;

  /// Initializes optional [Controller] variables that can be used for
  /// _refreshing and error displaying.
  /// This method is called automatically by the mounted `View`. Do not call.
  set globalKey(GlobalKey<State<StatefulWidget>> key) => _globalKey = key;

  /// global key
  GlobalKey<State<StatefulWidget>> get globalKey => getStateKey();

  /// Retrieves the [BuildContext] associated with the `View`. Will throw an
  /// error if initController() was not called prior.
  @protected
  ...
  /// Initialize the listeners inside the the [Controller]'s [Presenter].
  /// This method is called automatically inside the
  ...
  /// On iOS, this state corresponds to an app or
  /// the Flutter host view running in
  /// the foreground inactive state. Apps transition
  /// to this state when in a phone call,
  /// responding to a TouchID request, when entering the app
  /// switcher or the control center,
  ...
  /// Called when the application is visible and is responding to the user i.e.
  /// in the foreground and running.
  /// ```dart
  ///     class MyController extends Controller {
  ///       @override
  ...
  /// When the application is in this state, the engine is still running
  /// but not attached to any view.
  ///
  /// ```dart
  ///     class MyController extends Controller {
  ///       @override
  ...
  /// When the view is in this context, it means that the view is about
  /// to be extracted from the widget tree, but it may be
  /// added again. Quoting the view `deactivate` docs
  /// from `https://api.flutter.dev/flutter/widgets/State/deactivate.html`:
  ///
  /// ```The framework calls this method whenever it removes
  ///   this State object from the tree.
  ///   In some cases, the framework will reinsert the State object
  ///   into another part of the tree
  ///   (e.g., if the subtree containing this State object is grafted
  ///   from one location in the tree to another).
  ///   If that happens, the framework will ensure
  ///   that it calls build to give the
  ///   State object a chance to adapt to its new location in the tree.
  /// ```
  ///
  /// So, this may be the correct lifecycle to remove any resources that
  /// depend on other widgets in the widget tree.
  ///
  /// Usage:
  ///
  /// ```dart
  ///     class MyController extends Controller {
  ///       @override
  ///       void onDeactivated() => print('View is about to be
  ///         deactivated and maybe disposed');
  ///     }
  /// ```
  void onDeactivated() {}

  /// Called before the view is reassembled.
  /// When this method is called on view life cycle on `reassemble`, and it
  /// guarantees that `build` lifecycle will
  /// be called. Quoting the docs:
  ///
  /// ```Most widgets therefore do not need to do
  ...
  ///
  /// Should be used when need to perform some action on
  /// View.didChangeDependencies life cycle.
  /// View.initViewState should be called before the actions
  /// you need to perform. Like didChangeDependencies, you can safely perform
  /// actions that depends on [BuildContext] here.
  ///
  /// ```dart
  ///     class MyController extends Controller {
  ///       @override
  ///       void onDidChangeDependencies() => print('View is about to
  ///         run didChangeDependencies life cycle');
  ///     }
  /// ```
  ...
  ///
  /// Should be used when need to perform some
  /// action on View.initState life cycle.
  ///
  /// ```dart
  ///     class MyController extends Controller {
  ///       @override
  ///       void onInitState() => print('View is about to
  ///           run initState life cycle');
  ///     }
  /// ```
  void onInitState() {}
}

/// Descibes Controlled builder
typedef ControlledBuilder<AnyController extends Controller> = Widget Function(
    BuildContext context, AnyController controller);

/// This is a representation of a widget that is controlled by a
/// [Controller] and needs to be re-rendered when
/// [Controller.shake] is triggered.
///
/// This was created to optimize the render cycle from
/// a [ViewState]'s widget tree.
///
/// When [Controller.shake] is called, only the ControlledWidgets
/// inside [ViewState.view] will be re-rendered.
///
/// Example:
///
/// ```dart
...
class ControlledWidgetBuilder<AnyControler extends Controller>
  ...
  /// Controller builder
  final ControlledBuilder<AnyControler> builder;

  @override
  Widget build(BuildContext context) => Consumer<AnyControler>(
      builder: (BuildContext context, AnyControler controller, _) =>
          builder(context, controller));
}
