...
/// [Params] Is the object passed to the usecase containing
/// all the needed parameters for the [UseCase]
///
/// The [UseCase] represents a business-level process.
/// It should be written purely in `Dart` and `MUST NOT` include any `Flutter`
/// code whatsoever. The `UseCase` is a part
/// of the `Domain` module of the application in the `Clean Architecture`.
///
/// Dependencies used by the [UseCase] must be injected by the [Presenter].
/// The [UseCase] is essentially an `Stream` managing class.
/// When the [execute()] function is triggered
/// by the [UseCase], an `Stream` is built using the `buildUseCaseStream()`
///  method, subscribed to
/// by the [Observer] passed, and passed any required [Params].
/// The [StreamSubscription] is then added to a [CompositeSubscription].
/// This is later disposed when `dispose()` is called.
///
/// When extended, the extending class should override [buildUseCaseStream()],
/// where the behavior and functionality
/// of the [UseCase] are defined. This method will return the `Stream` to be
/// subscribed to, and will fire events to the `Observer` in the [Presenter].
///
...
///       final StreamController<GetSponsorsUseCaseResponse>
...
/// ```
///
/// The dependencies injected into the [UseCase] `MUST` be in
/// the form of `abstract` classes only, in order
/// to act as interfaces. These `abstract` classes are known as
/// `Repositories`. The [UseCase] `MUST NOT` accept
/// anything as a dependency in the form of a class that contains any
/// implemented code according to the `Clean Architecture`.
///
/// The `abstract` repositories are defined in the `Domain` module and
/// implemented in either `Device` or `Data` modules.
/// They are then injected polymorphically into the [UseCase].
/// The repositories should be injected inwards from the
/// outermost layer `View -> Controller -> Presenter -> UseCase`.
///
/// For example, the below is a an `abstract` repository
